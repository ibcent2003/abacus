...
{
    public class GetPendingApprovalDutyListQuery : DataTableListRequestModel, IRequest<DataTableVm<PendingApprovalDutyDto>>
    {
        public class GetPendingApprovalDutyListQueryHandler : IRequestHandler<GetPendingApprovalDutyListQuery, DataTableVm<PendingApprovalDutyDto>>
        {
            ...
            public GetPendingApprovalDutyListQueryHandler(IMapper mapper, IApplicationDbContext context, ICurrentUserService currentUserService)
            {
                _mapper = mapper;
                _context = context;
            ...
            public async Task<DataTableVm<PendingApprovalDutyDto>> Handle(GetPendingApprovalDutyListQuery request, CancellationToken cancellationToken)
            {
                var userId = _currentUserService.GetUserId();
                var data = _context.VehicleSearchPools.Where(x => x.Status=="Pending" && x.OwnedBy==null).AsQueryable();
